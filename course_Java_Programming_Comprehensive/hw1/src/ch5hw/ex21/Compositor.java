...
public class Compositor {
    ...
    public BufferedImage weightedAdd(BufferedImage image1, BufferedImage image2, double weight) {
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                ...
                // add samples using different weights to create a new color
                int red3 = Math.min((int) (weight * red1) + (int) (weight2 * red2), MAX_VALUE);
                int green3 = Math.min((int) (weight * green1) + (int) (weight2 * green2), MAX_VALUE);
                int blue3 = Math.min((int) (weight * blue1) + (int) (weight2 * blue2), MAX_VALUE);
    ...
    public static void main(String[] args) {
        ...
        BufferedImage p3 = c.weightedAdd(p1, p2, 0.20);
        dk.drawPicture(p3, 0, 25);

        BufferedImage p4 = c.weightedAdd(p1, p2, 0.35);
        dk.drawPicture(p4, 0, 700);
    }
}

// manipulates & combines 2 images to produce a new image
// ch5_p148_pdf153, hw1_p190_pdf195_ex21
